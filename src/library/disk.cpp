...
void
Disk::open(const char* path, size_t nblocks)
{
  ...
  // ftruncate is used to truncate or extend the file described by the FileDescriptor
...
// called when the object goes out of scope or it is explicitly deleted
Disk::~Disk()
{
  if (FileDescriptor > 0) {
...
void
Disk::sanity_check(int blocknum, char* data)
{
  char what[BUFSIZ];
...
void
Disk::read(int blocknum, char* data)
{
...
void
Disk::write(int blocknum, char* data)
{
  sanity_check(blocknum, data);
