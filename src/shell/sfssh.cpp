...
void
do_debug(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_format(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_mount(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_cat(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_copyout(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_create(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_remove(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_stat(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_copyin(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);
void
do_help(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2);

bool
copyout(FileSystem& fs, size_t inumber, const char* path);
bool
copyin(FileSystem& fs, const char* path, size_t inumber);
...
main(int argc, char* argv[])
{
  ...
  while (true) {
    ...
    // if no such type of input is provided than continue
    if (args == 0) {
      continue;
    }
...
do_format(Disk& disk, FileSystem& fs, int args, char* arg1, char* arg2)
{
  ...
  // just delete the existing data from the disk and update the SuperBlock
  if (fs.format(&disk)) {
...
copyout(FileSystem& fs, size_t inumber, const char* path)
{
  ...
  size_t offset = 0;
  while (true) {
    ssize_t result = fs.read(
      inumber,
      buffer,
      sizeof(buffer),
      offset); // read returns the number of the bytes that has been read
    if (result <= 0) {
      break;
    }
